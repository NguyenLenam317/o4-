      ...
      <Card className={`shadow-md ${className}`}>
        <CardContent className="p-6 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center min-w-0">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 flex-grow min-w-0">
              {[1, 2, 3, 4].map((i) => (
    ...
    <Card className={`shadow-md ${className}`}>
      <CardContent className="p-6 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center min-w-0">
          <div className="text-center md:text-left md:mr-6 mb-4 md:mb-0">
            <div className="flex items-center justify-center md:justify-start">
              <span className="material-icons text-5xl text-primary-dark flex-shrink-0">{weatherIcon}</span>
              <span className="text-4xl font-light ml-2">{Math.round(data.current.temperature)}°C</span>
            </div>
            <p className="text-neutral-500 weather-text">{data.current.weatherDescription}</p>
            <p className="text-sm text-neutral-600 mt-2 weather-text">Feels like {Math.round(data.current.feelsLike)}°C</p>
          </div>
          
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 flex-grow min-w-0">
            <div className="flex items-center">
              <span className="material-icons text-neutral-500 mr-2 flex-shrink-0">water_drop</span>
              <div>
                <p className="text-sm text-neutral-500 weather-text">Humidity</p>
        ...
        <div className="mt-6">
          ...
          <div className="flex overflow-x-auto py-2 justify-between">
            {(() => {
              ...
              if (data.hourly && data.hourly.time) {
                for (let i = 0; i < data.hourly.time.length; i++) {
                  ...
                  if (hour >= nextHour || time.getDate() > new Date().getDate()) {
                    ...
                    const temp = Math.round(data.hourly?.temperature?.[i] || 0);
                    const code = data.hourly?.weatherCode?.[i] || 0;
                    const icon = code in weatherIcons ? weatherIcons[code] : "help_outline";
                    
                    hourlyItems.push(
                      <div key={timeStr} className="flex flex-col items-center mx-auto min-w-[60px] px-2">
                        <p className="text-sm text-neutral-500">{displayHour}</p>
                        <span className="material-icons text-lg my-2 flex-shrink-0">{icon}</span>
                        <p className="text-sm font-medium weather-text">{temp}°C</p>
                      </div>
                    );
