...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    // Initialize with the demo user account if needed
    // The salt would normally be randomly generated
    const salt = "demo_salt";
    const demoPassword = "password123";
    
    // Create a demo user with a simple password
    this.users.set(0, {
      id: 0,
      username: "demo_user",
  ...
  async getUserByUsername(username: string): Promise<User | undefined> {
    // Find user by username using Array.from to convert map values to array
  ...
  async createUser(user: InsertUser): Promise<User> {
    ...
    // Hash the password
    let hash = user.password; // Default if crypto fails
    try {
